...
const ShopsList: React.FC = () => {
    ...
    debounce(async (query: string, location: string) => {
      ...
      try {
        ...
        if (results && results.length > 0) {
          ...
          // If no database results, try filtering the current shops
          if (shops.length > 0) {
  ...
  const fetchShops = async () => {
    try {
      ...
      if (Array.isArray(response) && response.length > 0) {
        ...
        // Empty response but API is working
        setShops([]);
      ...
      if (error instanceof Error && (
        ...
        const mockShops: Shop[] = [
          ...
          },
          {
            ...
            description: 'Trendy fashion boutique offering the latest styles in clothing, shoes, and accessories for men and women.',
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (results && results.length > 0) {
        ...
        const filteredResults = (Array.isArray(allShops) ? allShops : []).filter(shop => {
          const matchesSearch = shop.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
                               shop.description.toLowerCase().includes(searchQuery.toLowerCase());
          const matchesLocation = !locationFilter || 
                                 (shop.city && shop.city.toLowerCase().includes(locationFilter.toLowerCase())) ||
                                 (shop.country && shop.country.toLowerCase().includes(locationFilter.toLowerCase()));
          return matchesSearch && matchesLocation;
        });
  ...
  const handleSearchResultClick = (shop: Shop) => {
    setSearchQuery(shop.name);
    setLocationFilter(shop.city || '');
    setShowSearchResults(false);
    // Optionally navigate to the shop or update the main results
    setShops([shop]);
  };
...
const ShopCard: React.FC<{ shop: Shop }> = ({ shop }) => (
  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md hover:shadow-lg transition-shadow overflow-hidden group border border-gray-200 dark:border-gray-700">
    ...
    <div className="p-6">
      ...
      <div className="flex space-x-2">
        ...
        <Link
          to={`/shops/${shop.slug}`}
          className="flex-1 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-900 dark:text-white text-center py-2 px-4 rounded-lg transition-colors text-sm font-medium"
        >
          Visit Shop
        </Link>
      </div>
...
const ShopListItem: React.FC<{ shop: Shop }> = ({ shop }) => (
  <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md transition-shadow p-6 flex items-center space-x-4 group border border-gray-200 dark:border-gray-700">
    <Link to={`/shops/${shop.slug}`} className="flex items-center space-x-4 flex-1">
    ...
    <div className="flex flex-col items-end space-y-2">
      ...
      <div className="flex space-x-2">
        ...
        <Link
          to={`/shops/${shop.slug}`}
          className="bg-gray-100 hover:bg-gray-200 dark:bg-gray-700 dark:hover:bg-gray-600 text-gray-900 dark:text-white py-1 px-3 rounded text-sm transition-colors"
        >
          Visit
        </Link>
      </div>
